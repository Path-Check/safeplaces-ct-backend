...
exports.ingestUploadedPoints = async (req, res) => {
  ...
  let uploadedPoints = await uploadService.fetchPoints(accessCode);
  ...
  // Build hashes for all points in case that was passed in.
  // let point, coord, hash
  // let newPoints = []
  // for(point of uploadedPoints) {
  //   coord = pointService.coordinateToLatLng(point.coordinates)
  //   coord.time = point.time
  //   hash = await publicationFiles.hashPoint(coord)
...
exports.publishCases = async (req, res) => {
  ...
  if (organization) {
    ...
    if (publication) {
      ...
      let points = await caseService.fetchAllPublishedPoints(caseIds);
