...
const { gatekeeper } = auth;
const { userService } = require('./db');

const pkClient = new gatekeeper.JWKSClient(
  `${process.env.AUTH0_BASE_URL}/.well-known/jwks.json`,
);

const auth0Strategy = new gatekeeper.strategies.Auth0({
  jwksClient: pkClient,
  apiAudience: process.env.AUTH0_API_AUDIENCE,
});

const symJWTStrategy = new gatekeeper.strategies.SymJWT({
  algorithm: 'HS256',
...
module.exports = new gatekeeper.Enforcer({
  strategy: () => {
